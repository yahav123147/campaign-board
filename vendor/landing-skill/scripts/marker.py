#!/usr/bin/env python3
"""Hand-drawn red circle (default) or yellow highlighter around a number on a REAL screenshot.
Usage: marker.py in.png out.png x0 y0 x1 y1 [--style circle|yellow] [--crop X0 Y0 X1 Y1] [--resize W H]
...
if style=='yellow':
    d.rounded_rectangle((x0*S,y0*S,x1*S,y1*S),radius=8*S,fill=(255,230,0,150))
else:
...
ov=ov.resize(im.size,Image.LANCZOS); im=Image.alpha_composite(im,ov)
